...
| Modul          | Aufgabe |
| -------------- | ------- |
| `state.js`     | Der geteilte Zustand `S` (Filter, API-Antwort, gewaehlter Beleg, Erfassung, Caches je Immobilie/Beleg, Zeitraum-Feld, Kostenposition-Plan, Timer). `els` — einmal ausgelesene DOM-Referenzen. Konstanten `ALLE`, `ANLEGEN`, `MONATE`, `jetzt`. Die kurze Meldezeile `belegmeldung(text, art)` am Fuss des Pruefblatts (heisst bewusst NICHT `melde` — das ist der globale Toast aus `immo.js` mit anderer zweiter Angabe). |
