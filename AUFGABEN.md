...
| CCCXXI | **Baum: Status-Icon anklickbar (umhängen) + Legende + besseres Beleg-Icon**: (a) am zugeordneten Dokument hinten aufs Icon drücken → neu/anders verknüpfen (umhängen/lösen); (b) kleine Legende unter den Filtern: ✓ = einem Eintrag zugeordnet, Beleg-Icon = als Beleg angehängt; (c) den „Kreis mit Punkt" (⊙) durch ein kleines Beleg-/Datei-Symbol ersetzen | **offen** — `objekt.html` + Backend (Zuordnung lösen) |
