...
| CCCLXI | **Anteile in der Aufteilung inline anpassbar** — nach CCCLX zeigen abgeleitete Schlüssel nur €; der Nutzer braucht wieder die Möglichkeit, einzelne Anteile zu überschreiben (Bsp: Delta auf 0 → fällt raus, Rest verteilt sich neu). Aber OHNE „Anpassen"-Umschalten — direkt inline je Zeile, dezent, auto-speichernd. | **offen** — Frontend `zeitraum.html`: je Aufteilungszeile ein kleines Anteil-Feld (alle Schlüssel), PATCH `anteile`, €/% live neu; 0 lässt Partei rausfallen; „ableiten"-Link bleibt zum Zurücksetzen |
