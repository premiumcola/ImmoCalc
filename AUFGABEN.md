...
| CCCLXXVI | ~~**Scanner: Kantenerkennung + Overlay + Lupe (Paket 2)**~~ **teilweise erledigt** | Neue Kantenerkennung (Beleuchtungsausgleich → Schwellwert → größte Fläche → konvexe Hülle → 4 Ecken, Heftklammer-tolerant), Lupe beim Ziehen, kräftigeres/dunkleres Overlay in `kamerascan.js` integriert und live (lädt sauber, 0 Fehler). **Offen (Review, mittel):** auf sehr HELLEM Untergrund (heller Tisch) liefert die Erkennung den ganzen Bildrahmen (Rückfall, aber unpräzise); im Querformat/kurzen Viewport schrumpft die Vorschau zu stark. |
| CCCLXXV | **Scanner: grünes Overlay zu transparent / Kanten schlecht sichtbar** — die grüne Markierung über dem Dokument ist zu schwach zu sehen; das Dokument im Hintergrund darf ruhig kontrastreicher/abgedunkelter sein, damit die Kanten deutlich werden. | **offen** — Overlay-Deckkraft deutlich erhöhen, Außenbereich stärker abdunkeln, Kantenlinie kräftiger |
| CCCLXXIV | **Scanner: Lupe beim Ziehen der Eckpunkte** — beim Anfassen einer Ecke verdeckt der Finger genau die Stelle, die man treffen will. | **offen** — beim Ziehen eine vergrößerte Ausschnitt-Lupe (Kreis) einblenden, die den Bereich unter dem Finger zeigt (versetzt oder mittig), damit sich die Ecke genau setzen lässt |
