...
| CCLXXVI | ~~**Schief gescannte PDFs automatisch geradedrehen**~~ **erledigt** `42f2bf4` | `ocr._osd_drehung` (tesseract OSD) richtet Vorschau + KI-Bild auf; Endpunkt `POST /dokumente/{id}/geradedrehen` setzt die Seitenrotation dauerhaft + schreibt zurück in die Cloud (idempotent). **Achtung:** OSD-Richtung bei zerknitterten Foto-Scans nicht immer sicher (Doc 83 verifiziert) — ggf. manuelle 180°-Korrektur / bessere Erkennung nachziehen |
