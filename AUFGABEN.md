...
| CCLXXXIII | **Beleg in NK aus Nextcloud wählen**: in den Nebenkosten eines Objekts einen Beleg nicht nur per Foto, sondern **aus der verlinkten Nextcloud** auswählen und dann im **selben Erkennungs-/Zuordnungsdialog** wie im Dokumentenmodus fortfahren (Beleg evtl. schon im Warte-Archiv vorhanden, in der Masse aber nicht bearbeitet) | **offen** — `nebenkosten.html`/`zeitraum.html` + Backend (Cloud-Beleg-Picker → Erkennung) |
