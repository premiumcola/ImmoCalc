...
| CCLXVIII | **Beleg-Datum falsch → falscher NK-Zeitraum.** (a) ~~Heuristik nimmt jetzt das oberste **Kopf-Datum** statt des letzten unten~~ **erledigt** `4fce1f5`. (b) **KI-Auslese**: günstiges Claude-Modell (Haiku) liest den PDF-Text, extrahiert Belegdatum/Betrag/Klassifizierung zuverlässig | (a) live; (b) **in Arbeit** (Agent) — `kiauslese.py`, opt-in via `ANTHROPIC_API_KEY`, stummer Fallback ohne Key. **Datenschutz:** Beleg-Text geht nur mit gesetztem Key an Anthropic |
