...
| CCCLXXII | **„Aus Ablage" soll im richtigen Ordner starten** — bin ich in der NK-Abrechnung 2025 eines Objekts, soll die Dateiwahl direkt im passenden Nextcloud-Ordner öffnen (`…/(Objekt)/60_Nebenkosten/<Jahr>`), statt jedes Mal hin zu navigieren. | **offen** — ein nativer Datei-Dialog lässt sich technisch NICHT vorpositionieren → stattdessen den vorhandenen Nextcloud-Weg nutzen: In-App-Ordnerauswahl, die am richtigen Pfad startet (Endpunkte in `nextcloud.py`/Dokumenten-Router vorhanden) |
