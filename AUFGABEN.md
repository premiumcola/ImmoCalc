...
| Nr. | In einfachen Worten | Stand |
|---|---|---|
| CCXCVIII | **Grundstück-Ordnerstruktur schlanker**: ein Grundstück braucht nur die Ordner **Fotos Lage, Kommunikation, Kauf/Eigentum, Finanzierung, Nebenkosten, Steuer/Finanzamt** — plus neu **50 Pacht und Pächter**. Typ-abhängige Struktur | **offen** — `cloudkern.py`/`cloud.py` |
...
| CCC | **Nebenkosten-Bereich für Grundstücke wieder aufnehmen**: Grundstück hat Pacht-Einkünfte (jährlich) und die Grundsteuer als jährliche Nebenkosten — also einen kleinen NK-Bereich (im Kern eine Position: Grundsteuer). CCLXXXVI (NK bei Grundstück ausgeblendet) teilweise zurücknehmen | **offen** — `objekt.html` (NK/Zeitraum beim Grundstück wieder zeigen) |
