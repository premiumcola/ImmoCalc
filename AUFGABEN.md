...
| CCCLXXVIII | **Vorbestehende Miet-/Vermietet-Ungereimtheiten (aus dem Bubble-Review)** — (a) `vermietet` je Einheit nutzt `laufend = [m if bis_datum is None]` (ignoriert `ab_datum`, wertet jedes `bis_datum` als beendet) → weicht von `_laeuft` ab (befristet-aber-laufend = fälschlich „frei"). (b) `miete_monatlich` (Objektsumme) und `kaltmiete` im Detail-Payload rechnen den **Turnus nicht** heraus → €/m² in `objekt.html` bei vierteljährlicher Miete verdreifacht. (c) `.bubble`-CSS doppelt (index.html/objekt.html) → Kandidat für immo.css. | **offen** — additive Angleichung an `_laeuft`/`jahresbetrag`; Referenztests (test_api 39/58/65) bewusst anpassen; getrennt von CCCLXVII, weil es `vermietet` verschiebt |
