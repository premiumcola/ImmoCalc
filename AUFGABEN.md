...
| CCCXVI | **Dokumentenbaum nach ECHTEM Ordner** (nicht nach Kategorie): der Baum zeigte „99_Sonstiges", obwohl die Datei real in „40_Kauf_Eigentum_Finanzierung" liegt — verwirrend. Gruppierung aus `Dokument.pfad` (tatsächlicher Unterordner), damit die Ansicht 1:1 zu Windows/Nextcloud passt. Cloud-Box vereinfachen: nur „Ordner verknüpft" + Filter + Dateiübersicht; **fehlende Unterordner automatisch anlegen** (kein Knopf); leere-aber-vorhandene Ordner nicht extra zeigen | **offen** — `dokumente.py` (baum) + `objekt.html` (cloudZeigen) |
| CCCXVII | **Untergeordnete Belege eingerückt**: ein Info-Beleg (⊙) hängt an einer Hauptdatei → in Baum UND Rubriken (Notarverträge/Finanzamt) eingerückt unter der Hauptdatei/dem Eintrag darstellen (Hierarchie) | **offen** — `dokumente.py` (parent) + `objekt.html` |
