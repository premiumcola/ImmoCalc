...
| CCCLXXVIII | **Vorbestehende Miet-/Vermietet-Ungereimtheiten (aus dem Bubble-Review)** — (a) ~~`vermietet` je Einheit nutzte `laufend=[m if bis_datum is None]`~~ **behoben**: nutzt jetzt `_laeuft` (Einzug berücksichtigt, befristet-laufend = vermietet), test_api grün. (b) `miete_monatlich` (Objektsumme) und `kaltmiete` im Detail-Payload rechnen den **Turnus nicht** heraus → €/m² in `objekt.html` bei vierteljährlicher Miete verdreifacht. (c) `.bubble`-CSS doppelt (index.html/objekt.html) → immo.css. | **teilw. erledigt** — (a) live; (b)+(c) offen (kleine, gezielte Folge-Fixes) |
