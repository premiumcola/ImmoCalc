...
| CCLXXIX | **Dokumenten-Filter kompakter**: Kostenart-Filter zu lang (67 Chips) → **Dropdown mit den Bubbles darin** (aufklappbar). **Immobilien** ebenfalls als **Buttons** (nur Adresse + Immo-Icon), statt Auswahlfeld | **offen** — Frontend `eingang.html` |
| CCLXXX | **Kostenart-Dubletten zusammenfassen**: `grundsteuer/Grundsteuer`, `muell/Muell/Müll`, `Zaehlerstand/Zählerablesung`, `Materialien/Material`, `Messdienste/Messdienst`, `Heizung/Warmwasser`-Varianten, `Wasser/Abwasser`, `Haftpflichtversicherung→Gebäudehaftpflicht`, `Betriebskosten/Nebenkosten gesamt→Nebenkosten` … → kanonische Kostenart. Facette + Filter + Bestand normalisieren (67 → ~42) | **offen** — Backend `kostenarten.py` + `dokumente.py`, Batch-Endpunkt |
| CCLXXXI | **NK-Belegeintragungen vor 2025 wieder raus**: die automatisch erzeugten (vorläufigen) NK-Kostenpositionen zu Abrechnungszeiträumen **vor 2025** entfernen, Belege zurück ins Warten. Nutzer prüft Vor-2025 selbst / nimmt sie gar nicht auf („das ist zu viel") | **offen** — Backend, einmaliger Lauf |
| CCLXXXII | **Bulk „ins Warte-Archiv"**: Option im Dokumenteneingang, **alles aktuell Gefilterte** gesammelt zurück ins Warten zu schicken (Zuordnung lösen + vorläufige Orange-Entwürfe entfernen, Status „neu") | **offen** — `dokumente.py` (Sammel-Endpunkt) + `eingang.html` (Knopf) |
