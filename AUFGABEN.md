...
| CCCIX | **Zielrubrik beim Zuordnen wählbar (auch „Sonstiges")**: eine Kaufvertrag-Bestätigung liegt unter „Sonstiges", gehört aber zu **Notarverträge** — beim Zuordnen die Rubrik wählen können, nicht starr aus der Kategorie ableiten | **offen** — `objekt.html` (Zuordnen-Dialog) + `dokumente.py` (Zieltyp als Parameter) |
| CCCX | **Kostenposition oder Info-Beleg**: beim Zuordnen fragen, ob daraus eine **Kostenposition** (Betrag, € pro Jahr/Monat) wird oder nur ein **Info-Beleg** (z. B. Bestätigung) — Info-Belege hängen ohne Betrag am Eintrag | **offen** |
| CCCXI | **Belege an einen bestehenden Eintrag hängen**: die Bestätigung gehört an *den* Kaufvertrag, wenn es ihn gibt — im Baum per Drag & Drop zwei Dateien/einen Beleg auf einen Eintrag zusammenziehen | **offen** |
| CCCXII | **Wiederkehrendes staffeln statt stapeln**: Grundsteuer der letzten 10 Jahre = **eine** Zeile mit Jahres-Staffelung, nicht 10 Zeilen. Gilt für alle wiederkehrenden Positionen (wird bei den Häusern noch wichtiger) | **offen** — `objekt.html` (Zeilen je Art gruppieren) |
