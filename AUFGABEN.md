...
| CCXCIX | **Dokumente im Objekt anzeigen + zuordnen**: in der Objektansicht (am PC) die in den Unterordnern liegenden PDFs zeigen und einem Bereich zuordnen können (z. B. Steuer & Zahlungen), gern per Drag & Drop draufziehen. **Präzisiert (Dokumentenbaum):** Die Ablage als **farbiger Hierarchie-Baum** — Oberkategorien (passend zu den Rubriken unten, gleiche Farbe + gleiches Icon), darunter „wie Früchte am Baum" die zugehörigen Dokumente in derselben Farbe. **Oben Filter-Chips** mit denselben Icons/Gruppen, dynamisch wie bei den anderen Immobilien; Klick zeigt die Dokumente unter dem jeweiligen Oberordner. **Grau = noch keinem Eintrag zugeordnet** → von dort einem Eintrag zuordnen (Drag & Drop wäre schön) | **offen** — `objekt.html` + Backend (Doku-Liste je Objekt inkl. „zugeordnet an Eintrag?") |
