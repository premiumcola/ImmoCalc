...
| CCCXLI | **NK-Verteilung/Sankey strikt je EINHEIT** — rechte Seite des Kostenfluss-Diagramms mischt Bewohner/Parteien (Heuser-Teubert, Nicklas, Delta Wohnbau GmbH) UND Einheiten (Wohnug 1.OG, Studio 1.OG); teils erscheint dieselbe Sache doppelt (Partei „Roman" + Einheit „Studio 1.OG"). Soll IMMER einheitenbasiert dargestellt werden; Bewohner hier nicht auflisten. | **offen** — Ursache in `auswertung.py` (`_einheiten_zahlen`/`sankey`)/`cashflow.py`: Knoten-Label = Partei statt Einheit (lose Mietverhältnisse). Fix: rechte Sankey-Knoten strikt Einheit-Bezeichnung; Engine-Referenztests grün halten |
