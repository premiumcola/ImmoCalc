...
| CCXCIX | **Dokumente im Objekt anzeigen + zuordnen**: in der Objektansicht (am PC) die in den Unterordnern liegenden PDFs zeigen und einem Bereich zuordnen können (z. B. Steuer & Zahlungen), gern per Drag & Drop draufziehen. **Präzisiert:** die verknüpfte Ordnerübersicht als **Gliederung/Struktur** anzeigen, darin die **noch nicht zugeordneten** PDFs (grau); bereits zugeordnete ausblenden, damit es übersichtlich bleibt. Wenn nicht sinnvoll, Besseres vorschlagen | **offen** — `objekt.html` + Backend (Ordnerbaum + offene PDFs) |
