...
| CCLXXXVIII | **Immo-Filter-Buttons im Dokumenteneingang auf eine Zeile kürzen**: statt der langen Namen kurze Codes — erste 3 Buchstaben der Straße + Hausnummer (Laufer 5 → „lau 5", Tauchersreuther 5 → „tau 5"), Grundstück ~3 Elemente der Nummer. Spart die 5 Zeilen Höhe | **offen** — `eingang.html` |
| CCLXXXIX | **Leeren grünen Kasten oben im Dokumenteneingang entfernen**: die mint-grüne `.bilanz`-Leiste steht leer unter „Beleg abfotografieren" | **offen** — `eingang.html` |
| CCLXXXVI | **Bei Grundstücken Versicherung + Kredite/Bausparen ausblenden**: ein Grundstück (Eckenhaid) braucht keine Gebäudeversicherung und keinen Kredit-/Bausparblock — diese Rubriken dort raus | **offen** — `objekt.html` (`istGrundstueck()`) |
| CCLXXXVII | **Neue Rubrik „Notarverträge" bei allen Objekten**: Kaufvertrag/Auflassung/Grundschuldbestellung etc. mit Notar, URNr, Datum, Betrag, Beteiligten — als eigener Stammdaten-Bereich bei jedem Objekt (auch Grundstück) | **offen** — neues Modell `Notarvertrag`, `stammdaten.py`-Registrierung, `objekt.html`-Bereich |
